...
stmnt(Node *n)
{
	...
	case OIF:
		...
		if(r && r->type == OELSE) {
			stmnt(r->left);
			truedone = instruction(ABR, ZeroN, ZeroN, ZeroN);
	...
	case ODWHILE:
		instruction(ABR, ZeroN, ZeroN, ZeroN);
		enter = ipc;
		instruction(ABR, ZeroN, ZeroN, ZeroN);	/* Continue */
		pushjmp(&contstack);
		instruction(ABR, ZeroN, ZeroN, ZeroN); /* Break */
		pushjmp(&brkstack);
		...
		instruction(ABR, ZeroN, ZeroN, ZeroN); /* Loop */
		label(ipc, loop->pc+1);
...
switchcode(Node *n)
{
	...
	instruction(ABR, ZeroN, ZeroN, ZeroN);		/* Entry */
	enter = ipc;
	instruction(ABR, ZeroN, ZeroN, ZeroN); 	/* Break */
	pushjmp(&brkstack);
	...
	instruction(ABR, ZeroN, ZeroN, ZeroN); 	/* Done break */
	label(ipc, brkstack->i->pc);
...
parcode(Node *n)
{
	...
	for(i = 0; i < cnt-1; i++) {
		...
		instruction(ACMP, &retr, ZeroN, con(i));
